...
def run(cmd, check=True, capture_output=False, text=True):
    """Run a shell command and print it. Returns CompletedProcess."""
...
#         return
#     subnet_data = vpc_data["subnets"][subnet_name]
#     if "apps" not in subnet_data or not subnet_data["apps"]:
#         print(f"[INFO] No apps deployed in subnet '{subnet_name}'.")
#         return
#     for port, app in list(subnet_data["apps"].items()):
#         pid = app.get("pid")
#         if not pid:
#             continue
#         print(f"[INFO] Stopping app on port {port} (PID: {pid})...")
